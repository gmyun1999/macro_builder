...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        ...
        # (Already included nested_function_3 above)
        ...
        tests = [
            ...
            # Test 2: FileSystemBlock RENAME action
            {
                ...
                "command": service.convert_file_system_block_to_str_code(
                    fs_block_2 := FileSystemBlock(
                        block_type=BlockType.FILE_SYSTEM_BLOCK,
                        target=FileSystemType.FOLDER,  # FOLDER 유지
            ...
            # Test 11: Circular Reference between func_nested_1 and func_nested_2
            {
                "name": "Test 11: Circular Reference between func_nested_1 and func_nested_2",
                "command": None,  # 순환 참조는 별도로 처리
                "setup": [
                    # func_nested_1에 func_nested_2 참조 추가
                    lambda: nested_function_1.blocks[0].body.append(
                        ReferenceBlock(
                            block_type=BlockType.REFERENCE_BLOCK,
                            reference_id="func_nested_2",
                        )
                    ),
                    # func_nested_2에 func_nested_1 참조 추가하여 순환 참조 생성
                    lambda: nested_function_2.blocks.append(
                        ReferenceBlock(
                            block_type=BlockType.REFERENCE_BLOCK,
                            reference_id="func_nested_1",
                        )
